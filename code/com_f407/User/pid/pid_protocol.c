  ...
  while(1)
  {
    ...
    switch (cmd_type)
    {
      ...
      case SET_P_I_D_CMD:
      {
        ...
        PID_set_pid(&pid, p_temp, i_temp, d_temp);    // 设置 P I D
      }
      break;
      ...
      case SET_TARGET_CMD:
      {
        ...
        PID_set_target(&pid, actual_temp);    // 设置目标值
      }
      break;
      ...
      {				
        uint32_t temp = COMPOUND_32BIT(&frame_data[13]);     // 周期数
				if(temp > 60) temp = 60;	// 最多为60ms
				if(temp == 0) temp = 1;		// 最少为1ms
        __HAL_TIM_SET_AUTORELOAD(&htim3, temp*1000);
      }
      break;
